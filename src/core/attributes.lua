...
local function normalize_attribute(attribute)
    ...
    return attribute ~= '' and attribute or nil
end

local function attribute_key(attribute)
    attribute = normalize_attribute(attribute)
    if not attribute then return nil end
    if BM.slug then return BM.slug(attribute) end
    return string.lower(attribute):gsub('[^%w]+', '_'):gsub('^_+', ''):gsub('_+$', '')
end
...
function BM.get_attribute(source)
    ...
    local direct = normalize_attribute(source.balatromon_attribute) or normalize_attribute(source.attribute)
    if direct then return direct end

    local ability_attribute = source.ability and normalize_attribute(source.ability.balatromon_attribute)
    if ability_attribute then return ability_attribute end

    local center = source.config and source.config.center
    if center and center ~= source then return BM.get_attribute(center) end
end

function BM.get_attribute_definition(attribute)
    attribute = normalize_attribute(attribute)
    if not attribute then return nil end
    local registered = BM.attributes[attribute_key(attribute)]
...
function BM.add_attribute_badge(card, badges, center)
    local attribute = BM.get_attribute(card) or BM.get_attribute(center)
    if not attribute then return end
    local def = BM.get_attribute_definition(attribute)
    badges[#badges + 1] = create_badge(def.badge_label, def.badge_colour, def.text_colour, def.badge_scale)
end
...
function BM.has_attribute(source, attribute)
    ...
    if not current or not wanted then return false end
